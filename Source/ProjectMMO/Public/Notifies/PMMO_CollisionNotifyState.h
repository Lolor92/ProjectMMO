...
#include "PMMO_CollisionNotifyState.generated.h"

UENUM(BlueprintType)
enum class EPMMO_CollisionShape : uint8
{
	Sphere,
	Box,
	Capsule
...
struct FPMMO_NotifyRuntimeWindow
{
	FGuid WindowId;
...
UCLASS()
class PROJECTMMO_API UPMMO_CollisionNotifyState : public UAnimNotifyState
{
	GENERATED_BODY()
...
public:
	...
	EPMMO_CollisionShape CollisionShape = EPMMO_CollisionShape::Box;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Shape",
		meta=(EditCondition="CollisionShape==EPMMO_CollisionShape::Sphere", EditConditionHides))
	float SphereRadius = 30.f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Shape",
		meta=(EditCondition="CollisionShape==EPMMO_CollisionShape::Box", EditConditionHides))
	FVector BoxExtent = FVector(40.f, 40.f, 10.f);

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Shape",
		meta=(EditCondition="CollisionShape==EPMMO_CollisionShape::Capsule", EditConditionHides))
...
private:
	...
	TMap<TWeakObjectPtr<USkeletalMeshComponent>, FPMMO_NotifyRuntimeWindow> ActiveWindowsByMesh;
};
