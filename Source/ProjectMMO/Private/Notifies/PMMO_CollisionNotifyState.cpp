#include "Notifies/PMMO_CollisionNotifyState.h"
...
void UPMMO_CollisionNotifyState::NotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	float TotalDuration, const FAnimNotifyEventReference& EventReference)
{
	...
	FPMMO_NotifyRuntimeWindow& Window = ActiveWindowsByMesh.FindOrAdd(MeshComp);
...
void UPMMO_CollisionNotifyState::NotifyTick(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
...
void UPMMO_CollisionNotifyState::NotifyEnd(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	Super::NotifyEnd(MeshComp, Animation, EventReference);
}

bool UPMMO_CollisionNotifyState::ShouldRunPredictedCollision(const AActor* OwnerActor) const
{
...
bool UPMMO_CollisionNotifyState::BuildTraceTransform(USkeletalMeshComponent* MeshComp, FTransform& OutTransform) const
{
...
void UPMMO_CollisionNotifyState::SweepCollision(USkeletalMeshComponent* MeshComp, const FTransform& PreviousTransform,
...
FCollisionShape UPMMO_CollisionNotifyState::MakeCollisionShape() const
{
	switch (CollisionShape)
	{
	case EPMMO_CollisionShape::Sphere:
...
bool UPMMO_CollisionNotifyState::HasAlreadyProcessedTarget(USkeletalMeshComponent* MeshComp, AActor* TargetActor) const
{
	if (!MeshComp || !TargetActor) return true;

	const TWeakObjectPtr<USkeletalMeshComponent> MeshKey(MeshComp);
	const FPMMO_NotifyRuntimeWindow* Window = ActiveWindowsByMesh.Find(MeshKey);
...
void UPMMO_CollisionNotifyState::MarkTargetProcessed(USkeletalMeshComponent* MeshComp, AActor* TargetActor)
{
	if (!MeshComp || !TargetActor) return;

	const TWeakObjectPtr<USkeletalMeshComponent> MeshKey(MeshComp);
	FPMMO_NotifyRuntimeWindow& Window = ActiveWindowsByMesh.FindOrAdd(MeshKey);

	const TWeakObjectPtr<AActor> TargetKey(TargetActor);
	Window.ProcessedTargets.Add(TargetKey);
}

void UPMMO_CollisionNotifyState::TryPlayPredictedReaction(AActor* AttackerActor, AActor* HitActor) const
{
	...
	UPMMO_PredictionComponent* PredictionComponent = AttackerActor->FindComponentByClass<UPMMO_PredictionComponent>();
